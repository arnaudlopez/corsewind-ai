...
STATIC_FEATURES="${STATIC_FEATURES:-configs/ml_spot_static_features.dem_v1.json}"
LOG_ROOT="${LOG_ROOT:-$ML_ROOT/run_logs}"
LOG="${LOG:-$LOG_ROOT/rebuild_phys_v2_dem_2024_2026.log}"
STATUS="${STATUS:-$LOG_ROOT/rebuild_phys_v2_dem_2024_2026.status}"
PID="${PID:-$LOG_ROOT/rebuild_phys_v2_dem_2024_2026.pid}"
...
if [ ! -s "$STATIC_FEATURES" ]; then
  echo "missing static DEM features: $STATIC_FEATURES" >&2
  exit 71
fi
