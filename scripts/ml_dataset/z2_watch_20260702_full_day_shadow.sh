...
LOG_FILE="${LOG_FILE:-$LOG_DIR/20260702_full_day_shadow.log}"
TARGET_DATE="${TARGET_DATE:-2026-07-02}"
TARGET_END_UTC="${TARGET_END_UTC:-2026-07-02T17:00:00Z}"
...
OUTPUT_ROOT="${OUTPUT_ROOT:-$ML_ROOT/live_inference/collector_hindcast_shadow_unseen_20260702_full_day_v1}"
CASES_JSON="${CASES_JSON:-configs/ml_collector_shadow_cases_20260702_full_day_v1.json}"
...
run_suite() {
  log "coverage complete; running full-day shadow suite"
  ...
  log "full-day shadow suite finished: $OUTPUT_ROOT"
  if [[ "$RUN_POSTPROCESS" == "1" ]]; then
    log "running shadow suite postprocess"
    OUTPUT_ROOT="$OUTPUT_ROOT" TARGET_DATE="$TARGET_DATE" SUITE_VERSION="v1" RUN_LABEL="full_day_v1" \
...
log "watcher started target=$TARGET_END_UTC output=$OUTPUT_ROOT"
