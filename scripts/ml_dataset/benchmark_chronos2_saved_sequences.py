#!/usr/bin/env python3
"""Benchmark Chronos-2 on saved sequence benchmark cases."""
...
TARGETS = {
    "wind_mean_ms": {
        ...
        "output_prefix": "chronos2_univar_wind_mean_ms",
    },
    "gust_ms": {
        "context": "gust_ms",
        "actual": "actual_gust_ms",
        "output_prefix": "chronos2_univar_gust_ms",
    },
}
...
def summarize_metrics(frame: Any, deps: dict[str, Any], *, include_by_lead: bool = True) -> dict[str, Any]:
    ...
    for target, config in TARGETS.items():
        output = f"{config['output_prefix']}_p50"
        target_summary = {
            "chronos2_univar_p50": metric(np, frame, output, config["actual"]),
        }
        for model_name, column in (
            ("raw_nwp", f"raw_{target}"),
            ("hgb", f"hgb_{target}"),
...
def write_markdown(path: Path, result: dict[str, Any]) -> None:
    lines = [
        "# Chronos-2 Saved Sequence Benchmark",
        "",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--predictions-file", default="predictions_with_moirai.parquet")
    parser.add_argument("--output-root", type=Path)
    parser.add_argument("--run-id", default="chronos2_univariate_saved_sequence")
...
def main() -> None:
    ...
    for target, config in TARGETS.items():
        chronos_input = past[["item_id", "timestamp", config["context"]]].rename(columns={config["context"]: "target"})
        ...
        forecast = forecast.rename(columns={
            "predictions": f"{config['output_prefix']}_mean",
            "0.1": f"{config['output_prefix']}_p10",
            "0.5": f"{config['output_prefix']}_p50",
            "0.9": f"{config['output_prefix']}_p90",
        })
        keep = [
            "item_id",
            "timestamp",
            f"{config['output_prefix']}_mean",
            f"{config['output_prefix']}_p10",
            f"{config['output_prefix']}_p50",
            f"{config['output_prefix']}_p90",
        ]
        predictions = predictions.merge(forecast[keep], on=["item_id", "timestamp"], how="left")

    predictions.to_parquet(output_root / "predictions_with_chronos2_univariate.parquet", index=False)
    result = {
    ...
    (output_root / "benchmark_results_chronos2_univariate.json").write_text(json.dumps(result, indent=2, sort_keys=True, default=str) + "\n", encoding="utf-8")
    write_markdown(output_root / "benchmark_results_chronos2_univariate.md", result)
